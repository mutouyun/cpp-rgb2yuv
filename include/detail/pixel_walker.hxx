...
typename R2Y_ enable_if<(S == R2Y_ rgb_888 && F::iterator_size == 1 && F::is_block == 0)
...
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
    ...
    {
        do_sth(*reinterpret_cast<R2Y_ rgb_t (*)[F::iterator_size]>(& cur_pixel));
    }
}

template <R2Y_ supported S, typename F>
typename R2Y_ enable_if<(S == R2Y_ rgb_888 && F::iterator_size > 1 && F::is_block == 1)
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
...
template <R2Y_ supported S, typename F>
typename R2Y_ enable_if<(S == R2Y_ rgb_565 && F::iterator_size == 1 && F::is_block == 0)
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
    GLB_ size_t size = calculate_size<S>(in_w, in_h);
...
template <R2Y_ supported S, typename F>
typename R2Y_ enable_if<(S == R2Y_ rgb_565 && F::iterator_size > 1 && F::is_block == 1)
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
    assert((in_w % F::iterator_size) == 0);
    assert((in_h % F::iterator_size) == 0);
...
template <R2Y_ supported S, typename F>
typename R2Y_ enable_if<(S == R2Y_ rgb_555 && F::iterator_size == 1 && F::is_block == 0)
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
    GLB_ size_t size = calculate_size<S>(in_w, in_h);
    assert((size & 1) == 0); // in_size must be an even number
...
template <R2Y_ supported S, typename F>
typename R2Y_ enable_if<(S == R2Y_ rgb_555 && F::iterator_size > 1 && F::is_block == 1)
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
    assert((in_w % F::iterator_size) == 0);
    assert((in_h % F::iterator_size) == 0);
...
template <R2Y_ supported S, typename F>
typename R2Y_ enable_if<(S == R2Y_ rgb_444 && F::iterator_size == 1 && F::is_block == 0)
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
    GLB_ size_t size = calculate_size<S>(in_w, in_h);
...
template <R2Y_ supported S, typename F>
typename R2Y_ enable_if<(S == R2Y_ rgb_444 && F::iterator_size > 1 && (F::iterator_size & 1) == 0 && F::is_block == 0)
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
    GLB_ size_t size = calculate_size<S>(in_w, in_h);
    assert((size % 3) == 0); // 3 input bytes convert to 2 pixels (6 bytes)
...
template <R2Y_ supported S, typename F>
typename R2Y_ enable_if<(S == R2Y_ rgb_444 && F::iterator_size > 1 && (F::iterator_size & 1) == 0 && F::is_block == 1)
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
    assert((in_w % F::iterator_size) == 0);
    assert((in_h % F::iterator_size) == 0);
...
template <R2Y_ supported S, typename F>
typename R2Y_ enable_if<(S == R2Y_ rgb_888X && F::iterator_size == 1 && F::is_block == 0)
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
    GLB_ size_t size = calculate_size<S>(in_w, in_h);
...
template <R2Y_ supported S, typename F>
typename R2Y_ enable_if<(S == R2Y_ rgb_888X && F::iterator_size > 1 && F::is_block == 1)
>::type pixel_foreach(R2Y_ byte_t * in_data, GLB_ size_t in_w, GLB_ size_t in_h, F & do_sth)
{
    assert((in_w % F::iterator_size) == 0);
    assert((in_h % F::iterator_size) == 0);
